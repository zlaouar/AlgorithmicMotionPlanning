...
vector<vector<int>> motionPlan(vector<vector<int>> obstacles, int* start, int* goal, int scale_factor){
    ...
    while(1){ // run until goal found
        while(1){ // motion to goal
            
            xdist = qgoal[0] - qrobot[0];
            ydist = qgoal[1] - qrobot[1];
            //cout << "xdist: " << xdist << "ydist: " << xdist << ": " << ydist/abs(ydist) << endl;
            if(xdist != 0 && ydist != 0){
                if(switch_dir){ // move one step to the goal in x direction
                    qrobot[0] = qrobot[0] + (xdist/abs(xdist)); 
                    switch_dir = false;
                    direction = int(xdist/abs(xdist))*2;
                }
                else{ // move one step to the goal in y direction
                    qrobot[1] = qrobot[1] + (ydist/abs(ydist));
                    switch_dir = true;
        ...
        while(1){
            ...
            while(1){ // As long as robot is traveling along side of obstacle
                ...
                if(new_dist < prev_dist){// determine what point is the closest point to goal
                    ...
                    //temp = new_dist;
                    prev_dist = new_dist;
                    //cout << "PREV DIST: " << prev_dist << endl;
                }
                if(qrobot == qH){ // check if robot comes back to hit point
                    // Go to qL
            ...
            //return 0;
            // Go to qL
            while(qrobot!=qL){ // As long as robot is traveling along side of obstacle
                cout << "qrobot " << qrobot[0] << "," << qrobot[1] << endl;
                int top = map[height-qrobot[1]-1][qrobot[0]];
                int right = map[height-qrobot[1]][qrobot[0]+1];
                int bottom = map[height-qrobot[1]+1][qrobot[0]];
...
int main()
{
    ...
    vector<vector<int>> qpath_bug1_map1 = motionPlan(obstacles_map1, start_map1, goal_map1, scale_factor_map1);
    vector<vector<int>> qpath_bug1_map2 = motionPlan(obstacles_map2, start_map2, goal_map2, scale_factor_map2);

    
    
    // Write path trajectory of path 1 to a csv file
    ofstream bug1_map1_file;
    bug1_map1_file.open ("bug1_map1.csv");
    for(int i=0; i<qpath_bug1_map1.size(); i++){
        bug1_map1_file << qpath_bug1_map1[i][0] << "," << qpath_bug1_map1[i][1] <<"\n";
    }
    bug1_map1_file.close();

    // Write path trajectory of path 2 to a csv file
    ofstream bug1_map2_file;
    bug1_map2_file.open ("bug1_map2.csv");
    for(int i=0; i<qpath_bug1_map2.size(); i++){
        bug1_map2_file << qpath_bug1_map2[i][0] << "," << qpath_bug1_map2[i][1] <<"\n";
    }
    bug1_map2_file.close();
